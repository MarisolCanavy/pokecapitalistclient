...
type UnlockedProps = {
    ...
    showUnlock: boolean
    onCloseUnlock: () =>void
    deblockUnlock: (unlock:Palier)=>void
  }

function calculAllUnlocks(){
    return true
}

// TODO clickable si seuil faire fonction
export default function unlock({allunlocks, world, showUnlock, onCloseUnlock, deblockUnlock} : UnlockedProps){
    ...
    return (
        <div className="modal is-active absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 bg-light-gray border-medium-gray border-8 outline outline-8 outline-dark-gray max-w-xs text-center text-light-light-gray overflow-auto font-poke">
            <div className="grid grid-rows-2 grid-auto-rows grid-cols-3 place-items-center my-auto gap-16 my-10">
                ...
                {allunlocks.filter(unlock => unlock.unlocked === false).map((unlock) => 
                        <div className="text-color-font h-fit text-xl z-30 space-y-3">
                            ...
                            <div onClick={() => deblockUnlock(unlock)}>
                                <button
