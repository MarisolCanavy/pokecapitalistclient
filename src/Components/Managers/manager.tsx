...
export default function manager({managers, world, showManager, hireManager} : ManagerProps){
    ...
    return (
        <div className="modal is-active absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 bg-light-gray border-medium-gray border-8 outline outline-8 outline-dark-gray max-w-xs text-center text-light-light-gray overflow-auto font-poke">
            ...
            <button className="w-3/12 py-2 bg-light-gray text-xl border-medium-gray border-4 outline outline-4 outline-dark-gray text-center text-color-font" onClick={() => showManager = false}>Close
            </button>
        </div>
   );
}
